...
add_sub_imm = ARM64Instruction(
    ...
    instr_asm_regex=r"(add|sub)(s)?\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))(\s*,\s*lsl\s*#12)?",
...
mov_wide = ARM64Instruction(
    ...
    instr_asm_regex=r"movz?\s+(x|w)(\d{1,2})\s*,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))",
...
comp_branch_imm = ARM64Instruction(
    ...
    instr_asm_regex=r"cb(n)?z\s+(x|w)(\d{1,2})\s*,\s*label\s+//\s+\(label is (-?\d+) bytes away from pc\)",
...
sudiv = ARM64Instruction(
    ...
    instr_asm_regex=r"(s|u)div\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})",
...
logical = ARM64Instruction(
    ...
    instr_asm_regex=r"(and|orr|eor|ands|bic|orn|eon|bics)\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})(\s*,\s*lsl\s*#((0x[0-9a-f]+|0b[0-1]+|\d+)))?",
...
add_sub_reg = ARM64Instruction(
    ...
    instr_asm_regex=r"(add|sub)(s)?\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})(\s*,\s*lsl\s*#((0x[0-9a-f]+|0b[0-1]+|\d+)))?",
...
muls = ARM64Instruction(
    ...
    instr_asm_regex=r"(madd|msub|smaddl|umaddl|smsubl|umsubl|smulh|umulh)\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})(\s*,\s*(x|w)(\d{1,2}))?",
...
ldp_stp = ARM64Instruction(
    ...
    instr_asm_regex=r"(stp|ldp)\s+(x|w)(\d{1,2})\s*,\s*(x|w)(\d{1,2})\s*,\s*\[\s*x(\d{1,2})\s*((\])|(,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))\s*\])|(,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))\s*\]!)|(\]\s*,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))))",
...
ldr_str_uimm_offset = ARM64Instruction(
    ...
    instr_asm_regex=r"(ldr|str)(s)?(b|h|w)?\s+(x|w)(\d{1,2})\s*,\s*\[\s*x(\d{1,2})(\s*,\s*#((0x[0-9a-f]+|0b[0-1]+|\d+)))?\s*\]",
...
ldr_str_pre_post_idx = ARM64Instruction(
    ...
    instr_asm_regex=r"(ldr|str)(s)?(b|h|w)?\s+(x|w)(\d{1,2})\s*,\s*\[\s*x(\d{1,2})\s*((,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))\s*\]!)|(\]\s*,\s*#((-)?(0x[0-9a-f]+|0b[0-1]+|\d+))))",
...
ldr_str_reg_offset = ARM64Instruction(
    ...
    instr_asm_regex=r"(ldr|str)(s)?(b|h|w)?\s+(x|w)(\d{1,2})\s*,\s*\[\s*x(\d{1,2})\s*,\s*x(\d{1,2})\s*\]",
